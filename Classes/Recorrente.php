...
class Recorrente
{
    ...
    {
        $query = $this->conexao->prepare("SELECT *, valor_total as valor FROM parcelado WHERE para = :para");
        #$query->bindValue(':ativo', '1');
        $query->bindValue(':para', $conta);
        $query->execute();
        $resUm = $query->fetchAll(\PDO::FETCH_ASSOC);

        $query = $this->conexao->prepare("SELECT *, valor_total as valor FROM parcelado WHERE de = :de");
        #$query->bindValue(':ativo', '1');
        ...
        $query = $this->conexao->prepare("SELECT * FROM recorrente WHERE para = :para");
        #$query->bindValue(':ativo', '1');
        $query->bindValue(':para', $conta);
        $query->execute();
        $resUm = $query->fetchAll(\PDO::FETCH_ASSOC);

        $query = $this->conexao->prepare("SELECT * FROM recorrente WHERE de = :de");
        #$query->bindValue(':ativo', '1');
        $query->bindValue(':de', $conta);
        $query->execute();
        $resDois = $query->fetchAll(\PDO::FETCH_ASSOC);
    ...
    public function cancelarRecorrente($pid)
    {
