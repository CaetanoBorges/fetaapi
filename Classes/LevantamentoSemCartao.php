...
class LevantamentoSemCartao
{
    ...
    public function contaBalancoTipo($telefone)
    {
        ...
        $query = $this->conexao->prepare("SELECT tipo, balanco, identificador FROM cliente WHERE identificador = :id");
            $query->bindValue(':id', $cliente_identificador);
            $query->execute();
            $res = $query->fetch(\PDO::FETCH_ASSOC);
            return ["identificador_conta" => $res["identificador"], "tipo" => $res["tipo"], "saldo" => $res["balanco"]];
    }
